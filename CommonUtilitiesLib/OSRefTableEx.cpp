...
OSRefTableEx::OSRefEx * OSRefTableEx::Resolve(const string& keystring)//引用对象，存在返回指针，否则返回NULL
{
...
OS_Error OSRefTableEx::Release(const string& keystring)//释放引用
{
	OSMutexLocker locker(&m_Mutex);
	if (m_Map.find(keystring) == m_Map.end())//不存在
		return EPERM;
...
OS_Error OSRefTableEx::Register(const string& keystring, void* pobject)//加入到map中
{
	Assert(pobject != NULL);
	if (pobject == NULL)
		return EPERM;
...
OS_Error OSRefTableEx::UnRegister(const string& keystring)//从map中移除，必须引用计数为0才允许移除
{
	OSMutexLocker locker(&m_Mutex);
